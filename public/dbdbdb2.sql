...
CREATE TABLE IF NOT EXISTS `mydb`.`StockWithdrawal` (
  ...
  PRIMARY KEY (`withdrawalId`, `entryId`, `withdrawnBy`, `authorizedBy`),
  ...
  CONSTRAINT `fk_StockWithdrawal_Users1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_StockWithdrawal_StockEntry1`
    FOREIGN KEY (`entryId`)
...
CREATE TABLE IF NOT EXISTS `mydb`.`ReturnExchange` (
  ...
  `transactionStatus` ENUM('refunded', 'replaced', 'denied', 'deleted') NOT NULL,
  `orderId` INT NOT NULL,
  `handledBy` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `mydb`.`ReturnExchangeInfo` (
  ...
  `exchangeProductId` INT NULL,
  `exchangeQuantity` INT NULL,
